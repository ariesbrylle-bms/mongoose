...
(function(){
  ...
  var productController = new Vue({ // eslint-disable-line
        ...
        data: {
            ...
            quantity: null,
            price: null,
            photo_path: null, // eslint-disable-line
            productId: 'none',
            products: [],
            ctr: 0
        },
        ...
        methods: {
            ...
            resetForm: function(){
                ...
                this.quantity = '';
                this.price = '';
                this.photo_path = ''; // eslint-disable-line
                this.productId = 'none';
                $('#img-upload').attr('src', '');
                $('input').val('');
            },
            ...
            onSubmit: function(){
                if(typeof $('#imgInp')[0].files[0] === 'undefined' && String(this.productId) === 'none'){
                    ...
                    var payload = {
                        ...
                        photo_path: productController.photo_path, // eslint-disable-line
                        productId: productController.productId
                    };

                    if(String(this.productId) === '' && String(this.productId) === String(null)){ // eslint-disable-line
                        // eslint-disable-line
                    } else {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if(String(response.status) === 'Success'){
                            productController.photo_path = response.path; // eslint-disable-line
                            var payload = {
                                sku: productController.sku,
                                name: productController.name,
                                description: productController.description,
                                quantity: productController.quantity,
                                price: productController.price,
                                photo_path: response.path, // eslint-disable-line
                                productId: productController.productId
                            };
                            ...
                            if(String(productController.productId) === 'none'){
                                ...
                                axios.put(`http://localhost:${PORT}/products/update/` + productController.productId, payload) // eslint-disable-line
                                    .then((res) => {
                                        if(String(res.data.status) === 'Success'){
                                            productController.resetForm();
                                            productController.fOnload();
            ...
            deleteProduct(id) {
                ...
                axios.delete(`http://localhost:${PORT}/products/delete/` + id)
                    .then((res) => {
                        ...
                        this.notification('error', 'Error while deactivating product. Please contact system administrator'); // eslint-disable-line
                    });
            },
            enableProduct(id) {
            ...
            getDetails: function(id){
                axios.get(`http://localhost:${PORT}/products/get/` + id)
                    .then((response) => {
                        ...
                        this.photo_path = response.data.photo_path; // eslint-disable-line
